  ...
  methods: {
    ...
    setIsGoing: function(item,flag){
      ...
      xhttp.onreadystatechange = function() {
        if (this.readyState == 4 ){
          if ( this.status == 200) {
            const guest_id = parseInt(this.responseText);
            const guest = item.guestlist.find(g=>g.guest_id === guest_id);
            guest.is_going = item.is_going;
          }
    ...
    mainpage_mount: function() {
      this.todaysEvents = [];
      this.upcomingEvents = [];
      // console.log("heloooooooo");
      this.mainpage_mount_guest();
      this.mainpage_mount_host();
      this.mainpage_mount_guest_upcoming();
      this.mainpage_mount_host_upcoming();


    },

    mainpage_mount_guest_upcoming: function() {
      let xhttp = new XMLHttpRequest();
      let vueInst = this;
      var obj = {id: this.profile.id};
      console.log("upcomingguest: profile id:", this.profile.id);
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {;
                // alert("updated mainpage");
                console.log("res in moun guest upcoming: ", JSON.parse(this.responseText));
                let temp = JSON.parse(this.responseText);

                for (let i=0; i<temp.length; i++) {
                  console.log("pushing");
                  vueInst.upcomingEvents.push(temp[i]);
                }

                console.log("upcomingevents in guest after pushing: ", vueInst.upcomingEvents);
            } else if (this.readyState == 4 && this.status >= 400) {
                alert("Failed to update mainpage");
            }
        };

        xhttp.open("POST", "/users/upcomingEventsGuest");
        xhttp.setRequestHeader("Content-type", "application/json");
        xhttp.send(JSON.stringify(obj));
    },

    mainpage_mount_host_upcoming: function() {
      let xhttp = new XMLHttpRequest();
      let vueInst = this;
      var obj = {id: this.profile.id};
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {;
                // alert("updated mainpage");
                console.log("res in moun host upcoming: ", JSON.parse(this.responseText));
                let temp = JSON.parse(this.responseText);
                ...
                console.log("todaysEvent in host after pushing: ", vueInst.upcomingEvents);
            } else if (this.readyState == 4 && this.status >= 400) {
                alert("Failed to update mainpage");
            }
        };

        xhttp.open("POST", "/users/upcomingEventsHost");
        xhttp.setRequestHeader("Content-type", "application/json");
        xhttp.send(JSON.stringify(obj));
    },

    mainpage_mount_guest: function() {
      ...
      var obj = {id: this.profile.id};
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {;
                // alert("updated mainpage");
                console.log("res in moun guest: ", JSON.parse(this.responseText));
                let temp = JSON.parse(this.responseText);

                for (let i=0; i<temp.length; i++) {
                  console.log("pushing");
                  vueInst.todaysEvents.push(temp[i]);
                }

                console.log("todaysEvent in host after pushing: ", vueInst.todaysEvents);
            } else if (this.readyState == 4 && this.status >= 400) {
                alert("Failed to update mainpage");
            }
        };

        xhttp.open("POST", "/users/todaysEventGuest");
        xhttp.setRequestHeader("Content-type", "application/json");
        xhttp.send(JSON.stringify(obj));

    },

    mainpage_mount_host: function() {
      let xhttp = new XMLHttpRequest();
      var vueInst = this;
      var obj = {id: this.profile.id};
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {;
              // alert("updated mainpage");
              console.log("res in moun host: ", JSON.parse(this.responseText));
              let temp = JSON.parse(this.responseText);
              ...
              console.log("todaysEvent in guest after pushing: ", vueInst.todaysEvents);
            } else if (this.readyState == 4 && this.status >= 400) {
                alert("Failed to update mainpage");
            }
        };

        xhttp.open("POST", "/users/todaysEventHost");
        xhttp.setRequestHeader("Content-type", "application/json");
        xhttp.send(JSON.stringify(obj));
    },
 ...
 */
function handleAuthClick(item) {
tokenClient.callback = async (resp) => {
  if (resp.error !== undefined) {
    throw (resp);
  }
  // document.getElementById('authorize_button').innerText = 'Refresh';
  await get_c_event(item);
};
...
async function get_c_event(item){
  ...
  xhttp.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
          ...
          console.log("Item id is: ", item.event_id);
          console.log("Length of even_list is: ", event_list.length);
          ...
          for (let i=0; i<event_list.length; i++){
            console.log("test");
            console.log("matching id", event_list[i].event_id);
            if (event_list[i].event_id == item.event_id){
              index = i;
            }
          }
          ...
          var title = event_list[index].title;
          var location = event_list[index].address_1 + ", " + event_list[0].address_2 + ", " + event_list[0].address_3;
          var desc = event_list[index].dStr;
          var date = event_list[index].event_date;
          date = date.substring(0, 11);
          var start = date + event_list[index].timestart + ".0";
          var end = date + event_list[index].timeend + ".0"
