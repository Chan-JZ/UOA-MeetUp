...
const HOST= 'https://wei-way-code50-87966966-q74vrwxx5h97jv-8080.githubpreview.dev/'
...
router.post('/eventGuestListUpdate', function(req, res, next) {
  console.log("in event guestlistupdate");
  console.log(req.body);
  ...
  req.pool.getConnection(function(error, connection) {
    ...
    connection.query(query, [req.body.guestID, req.body.eventID], function(error, rows, fields) {
      ...
      connection.query(query, [req.body.guestID], function(error, rows, fields) {
        ...
        let query = "SELECT u.first_name, u.last_name, e.* FROM Event e LEFT JOIN Hostlist h ON h.event_id = e.event_id LEFT JOIN User u ON host_id = u.user_id WHERE e.event_id = ?";  //new
        connection.query(query, [req.body.eventID], function(error, hrows, hfields) {
          connection.release();
          if (error) {
            console.log(error);
            res.end();
          ...
          const  hostname = hrows[0].first_name + " " + hrows[0].last_name;
          //const event_date = hrows[0].event_date;
          const eventstarttime = hrows[0].event_starttime;
          const eventendtime = hrows[0].event_endtime;
          const event_date = hrows[0].event_date;
          const stDate = event_date.toLocaleDateString("en-US", { day: 'numeric' })
            + "-"+ event_date.toLocaleDateString("en-US", { month: 'short' })
            + "-" + event_date.toLocaleDateString("en-US", { year: 'numeric' });
          ...
          req.emailService.sendMail({
            ...
            // html: `<h3>You are invited to an event. Please specify your availability by clicking on the
            // <a href=${link}>link</a></h3>`, // html body
            html:`You have been invited to an event
              on ${stDate} from ${eventstarttime} - ${eventendtime}.
              Please specify your availability by clicking on this <a href=${link}>link</a>`
          }, (err, info) => {
            if ( err ) {
              console.log(err);
            }
...
router.post('/usersPrivilegeUpdate', function(req, res, next) {
  console.log("in usersprivilege");
  console.log(req.body);
  req.pool.getConnection(function(error, connection) {
    if (error) {
      console.log(error);
    ...
    let query = "UPDATE User SET isAdmin = true WHERE user_id = ?";
...
// new using Promise to make async function
function getGuestlist (connection, event_id) {
  return new Promise((resolve, reject)=>{
    const query = "SELECT * FROM User INNER JOIN Guestlist ON user_id = guest_id WHERE event_id  = ?;";
    connection.query(query, [event_id], (error, rows)=>{
          if(error){
              return reject(error);
          }
          return resolve(rows);
    ...
    connection.query(query, async function(error, rows, fields) {
      ...
      try{
        for(var i=0;i<rows.length;i++){
          const item = rows[i];
          item.guestlist = await getGuestlist(connection, item.event_id);
        }
      }
      catch(e){
        connection.release();
      ...
      connection.release();      res.send(rows);
      res.end();
    });
  });
})
...
router.post('/todaysEventGuest', function(req, res, next) {
  // console.log(req.body)
  req.pool.getConnection(function(error, connection) {
    if (error) {
      console.log(error);
    ...
    let query = "SELECT * FROM Event INNER JOIN Guestlist ON Event.event_id = Guestlist.event_id INNER JOIN Hostlist ON Hostlist.event_id = Event.event_id INNER JOIN User ON User.user_id = Hostlist.host_id WHERE DATE(event_date) = CURDATE() AND Guestlist.guest_id = ?;";
    connection.query(query, [req.body.id], function(error, rows) {
      connection.release();
      if (error) {
        console.log(error);
        res.sendStatus(500);
...
router.post('/todaysEventHost', function(req, res, next) {
  // console.log(req.body);
  req.pool.getConnection(function(error, connection) {
    if (error) {
      console.log(error);
    ...
    let query = "SELECT * FROM Event INNER JOIN Hostlist ON Hostlist.event_id = Event.event_id INNER JOIN User ON User.user_id = Hostlist.host_id WHERE DATE(event_date) = CURDATE() AND Hostlist.host_id = ?;";
    connection.query(query, [req.body.id], function(error, rows) {
      connection.release();
      if (error) {
        console.log(error);
...
router.post('/upcomingEventsHost', function(req, res, next) {
  req.pool.getConnection(function(error, connection) {
    if (error) {
      console.log(error);
      res.sendStatus(500);
...
router.post('/upcomingEventsGuest', function(req, res, next) {
  console.log("upcomingeventguest id: ", req.body.id);
  req.pool.getConnection(function(error, connection) {
    if (error) {
      console.log(error);
      res.sendStatus(500);
