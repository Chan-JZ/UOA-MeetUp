...
// const CLIENT_ID = process.env.GOOGLE_OAUTH2_CLIENT_ID; //Niki's client id
const CLIENT_ID = '80995453411-gcpjlgvmb4o4a4oiotge2spqcn8co78r.apps.googleusercontent.com'; //WL's client id
...
router.get('/forgot-password', function(req, res) { // new for logout
  ...
  req.emailService.sendMail({
    ...
    html: `<a href="https://wei-way-code50-87966966-q74vrwxx5h97jv-8080.githubpreview.dev/reset-password.html?email=${email}">
...
router.get('/c_event', function(req, res, next) {
  req.pool.getConnection(function(error, connection){
    ...
    // const title = "WL has a big nutsack"
    // const locationStr = "My balls"
    // const dStr = "W&D is my 4th favorite course this sem"
    // const timestart = "2022-06-09T10:00:00.0"
    // const timeend = "2022-06-09T12:00:00.0"
    // const guestList = [
    //   {"email": "38985@jisedu.or.id"},
    //   {"email": "weilongsg@gmail.com"}
    // ]
    ...
    let query = "SELECT event_name AS title, address_1, address_2, address_3, event_description AS dStr, event_starttime AS timestart, event_endtime AS timeend, event_date, event_id FROM Event;";
...
router.get('/u_calendar', function(req, res, next) {
  req.pool.getConnection(function(error, connection){
    ...
    let query = "SELECT event_starttime AS start, event_endtime AS end, event_name AS title, event_date, Event.event_id FROM Event INNER JOIN Hostlist ON Event.event_id = Hostlist.event_id INNER JOIN User ON Hostlist.host_id = User.user_id;";
    // let query = "SELECT event_starttime AS start, event_endtime AS end, event_name AS title, event_date, Hostlist.event_name FROM Event INNER JOIN Hostlist ON Event.event_id = Hostlist.event_id INNER JOIN User ON Hostlist.host_id = User.user_id;";
